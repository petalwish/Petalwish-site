...
4.  **交互节奏 (Rhythm)**: 强调“静谧如海”的入水感，动效极长且丝滑，利用 3D 点云、毛玻璃容器与编辑器式冷灰底上的微光营造高精尖实验室的沉浸感。
...
- **背景 (Surface) — VS Code 质感灰黑**: `#1E1E1E` —— 对齐默认深色编辑器画布，中性灰黑、略偏冷，便于长时间阅读与衬托高亮与点云。
- **抬升面 (Surface Elevated)**: `#252526` —— 侧栏 / 面板层级感，用于毛玻璃容器下的叠层参考。
...
### 2.1 背景网格 (Editor Grid)

- **用途**: 全站画布铺一层极淡的正交网格，模拟 IDE 工作区底纹，增强「数字实验室」秩序感；须保持低调，不抢正文与 3D 主体。
- **CSS 变量建议**:
  - `--grid-size`: `20px`（与常见编辑器密度接近，可按断点微调）。
  - `--grid-line`: `rgba(255, 255, 255, 0.042)`（在 `#1E1E1E` 上近似 VS Code 底纹对比度）。
- **实现**: 在 `body` 上使用 `background-color: Surface` + 双层 `linear-gradient` 线网，`background-size` 对齐 `--grid-size`，`background-attachment: fixed` 使滚动时网格稳定如桌面。
...
*“嘿，AI，基于这份视觉规范文档，先帮我重写 Tailwind 配置文件，并为我的 Astro 项目创建一个全局的 Layout 骨架，确保背景网格与 VS Code 质感的灰黑 Surface（`#1E1E1E`）准确。”*